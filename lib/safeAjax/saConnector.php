...
// 
// 	version 4.5 from safeAjax v4 - added [UserData] user maintenance functions
...
$json = array(); $json['connectorVersion'] = 4.5; // these are the outputs
...
try { 
	...
	// send the functionality we support back to the safeAjax library
	$json['canDo_forgotUsername'] = $authenticatorObj->canDo_forgotUsername();
	$json['canDo_forgotPassword'] = $authenticatorObj->canDo_forgotPassword();
	$json['canDo_resetPassword']= $authenticatorObj->canDo_resetPassword();
	...
	// print "<!-- " .$e->getMessage(). " -->";
	$json['safeAjaxError'] = $e->getMessage();
	$json['authOK'] = true; // prevent authRequest
}
