...
function SafeAjax (options) {
	...
	// ensure object instatiation, thanks be to the ninjaResig
	if ( !(this instanceof SafeAjax) ) { return new SafeAjax(options); }
	...
	this.settings = sets; // public access

	if (! this.JSON) { // load JSON if not avail already
	...
	function credits() {
		...
		log("SafeAjax credits: this code would not have been possible without the hard work of the following developers: " +JSON.stringify(attribs)); 
	};
	credits();
	...
	this.syncLogout = function() {  // public access 
	...
	/*********** make a [blocking] call for logout to the backend ***********/
	this.syncLoggedIn = function() {  // public access 
	...
	this.syncLogin = function(username,password) {  // public access 
		
		this.syncLogout(); // ensure we are not us anymore
	...
	this.syncRequest = function(requestObj){  // public access
		...
		try {
			...
			if (json.safeAjaxError) { log(json.safeAjaxError); }
			if (! json.authOK) {
				if ( sets.handleLogin ) { // we would use our own login dialog
			...
			return fatal("commError:"+commError+ " (in syncRequest)");
		}
	};

	// ********** push a call to the backend without blocking ***********
	this.asyncRequest = function(requestObj,callbackPass,callbackFail){  // public access
		...
		try {
			...
			callOpts.complete = function(json, result){
				...
				try {
					...
					if (json.status == 404) {
						// the call failed, construct a return object to our consistancy
						var json = fatal("backend is missing or broken! (in asyncRequest)");
	...
	function prep4Logins(){
		...
		if (! sets.loginMissing) {
			...
			if ($("#SafeAjaxDiv").length == 0) { $("<div id='SafeAjaxDiv'>").appendTo("body"); }
			...
			$("#sa_Login").hide().jqm({ modal: true }); $("#sa_PWord").val("safeAjax");
		}
	...
	/*********** the queue for requests pending on a login ********** not yet in use */
	function loadQueue(callOpts){
		$([ this.loadQueue ]).queue("ajax", function(){
			this.catchSyncCall(jsonReturn);
			runQueue();
		});
	};

	/*********** run ajax queue after a successful login *********** not yet in use */
	function runQueue(){
		$.dequeue( this.loadQueue, "ajax" );
	};
